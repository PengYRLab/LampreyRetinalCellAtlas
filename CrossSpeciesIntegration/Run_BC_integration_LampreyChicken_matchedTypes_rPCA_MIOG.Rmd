...
seu<-subset(seu, idents=tmp[c(1,2)])
...
tmp<-seu@meta.data$subtype %>% as.character()

tmp[tmp %in% c("BP-4", "BP-14", "BP-19")]<-"RBC"
tmp[tmp %in% c("BP-1", "BP-9", "BP-22", "BP-18", "BP-3", "BP-13", "BP-10", "BP-21")]<-"ON BC"
tmp[tmp %in% c("BP-12", "BP-15", "BP-8", "BP-5", "BP-7", "BP-11", "BP-16", "BP-17", "BP-2", "BP-6", "BP-20")]<-"OFF BC"
...
```{r}
 ...
 Idents(seu)<-"subtype"
 seu<-subset(seu, idents=c("BP-14", "BP-4", "BP-9", "BP-6" ,"BP-2", "BP-11", "BP-15"))
```
...
table<-read.table(file="Lamprey__v__Chicken.tsv",
                  fill=TRUE,
                  sep="\t",
                  header=TRUE)
...
table<-GetBestOrthologuesBySD( ortho_group_table=table,
                              exp_mat=seu.chicken.1@assays$RNA@scale.data,
                              species="Chicken",
                              pattern=","
                              )
...
mat<-OrthogroupID2GeneSymbol(mat=mat,
                                  ...
                                  reference_species=c("Chicken", "Lamprey")
                                  )
...
ggsave("dimplot.integrated.subclass.pdf", width=15, height=5)
