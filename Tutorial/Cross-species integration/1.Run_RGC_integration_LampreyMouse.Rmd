...
dir<-"/Users/mumu/Documents/bioSoft/bioSoft/Rpackages/scML/IntegrationUsingOrthogroup_Pipeline_v0.1_20240321"
...
load("../seu.lamprey.rgc.r3_finalObject_archived_05192023_subtypeAdded.rda")
...
#seu<-subset(seu, nFeature_RNA>2000)
...
table<-read.table(file="/Users/mumu/Documents/research/LampreyRetina/analysis/OrthoFinder/Run_orthoFinder/run_lampreyProject/out_OrthoFinder_v1.1/Orthologues/Orthologues_Lamprey/Lamprey__v__Mouse.tsv",
                  fill=TRUE,
                  sep="\t",
                  header=TRUE)
...
Plot the cell-type composition in the clusters
```{r}
...
seu@meta.data$seurat_clusters<-factor(seu@meta.data$seurat_clusters, levels=c(1:length(unique(seu@meta.data$seurat_clusters))))

seu@meta.data$species<-as.vector(seu@meta.data$species)

col_vector

df<-seu@meta.data %>% as.data.frame()

#plot as proportion or percentage of cluster
p<-ggplot(df, aes(x=integrated_clusters, fill=species)) +
  geom_bar(position = "fill") +
  ylab("fraction") +
  xlab("") +
  #theme(text=element_text(8))+
  theme(axis.text.x = element_text(angle = 60, vjust = 0, hjust= 0)) +
  theme( axis.text = element_text()) +
  
  scale_fill_manual("species", values=c("mouse"="#E7298A", "lamprey"="#33A02C")) +
...
ggsave(file="CellCompostionBydonor.pdf", width=3, height=5)
...
```{r}
save(seu, file="RGC_integrated.mouse.lamprey.rda")
```
