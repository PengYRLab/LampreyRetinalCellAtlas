...
# This function aggregates the gene expressions in one orthogroup, the orthogroups are specified by the rowname
...
AggregateOrthoGroup<-function(exp_mat,
  ...
  # exp_mat <- seu.lamprey@assays$RNA@counts
  ...
  hash_orthogroup<-hash(keys=rownames(ortho_group_table), values=ortho_group_table[,column_species_index])
  
  hash_gene_in_expression <- hash(keys=rownames(exp_mat), values=1:nrow(exp_mat))
